...
fn main() -> unit {
  ...
  // println!("{}", p1.x); // ERROR: p1 used after move
}
...
fn use_pass_by_move() -> unit {
  ...
  // println!("{}", p3.x); // ERROR: p3 used after move
}

fn pass_by_ref(pt: ref Point) -> unit {
  println!("{}", pt.x); // OK to read
...
fn use_borrows() -> unit {
  ...
  println!("{}", p4.x);   // p4 still owned and usable

  pass_by_inout(p4); // Pass mutable reference
  println!("{}", p4.x);   // p4 still owned, value might have changed
}
